...
public struct Line
{
    ...
    public Line(Vector2 pointOnLine, Vector2 pointPerpendicular)
    {
        ...
        aproachSide = false;
        aproachSide = GetSide(pointPerpendicular);
    }
    ...
    {
        float yInterceptPerdincular = p.y - gradientPerpendicular * p.x;
        float intersectX = (yInterceptPerdincular - y_intercept) / (gradient - gradientPerpendicular);
